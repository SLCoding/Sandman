...
check for users which are logged in
	...
	def configure():
		configurable = []
		configurable.append(["usercheck", "max_usr", '0', "amount of users logged in. nevertheless it's going to sleep | -1 = disable"])
		configurable.append(["usercheck", "max_usr_local", '-1', "amount of users logged in local. nevertheless it's going to sleep | -1 = disable"])
		configurable.append(["usercheck", "max_usr_remote", '-1', "amount of users logged in remotely. nevertheless it's going to sleep | -1 = disable"])
		configurable.append(["usercheck", "idle_timeout", '1800', "time in sec. user which idle above this time are not counted | -1 = no timeout all user are counted"])
		return configurable
